...
public class OilSplashHandler : ActionTask
{
    ...
    {
        if (sophieObject.value == null || agent == null)
        {
            Debug.LogError("SophieObject or Agent is not assigned!");
            EndAction(false);
            return;
        }

        // Initialize Sophie's renderer and original color
    ...
    private void HandleOilHit()
    {
        ...
        agent.gameObject.AddComponent<CoroutineHelper>().StartCoroutineHelper(WaitAndReset());
    }
    ...
    // Helper MonoBehaviour to handle coroutines
    private class CoroutineHelper : MonoBehaviour
    {
        public void StartCoroutineHelper(System.Collections.IEnumerator coroutine)
        {
